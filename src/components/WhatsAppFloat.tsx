...
export default function WhatsAppFloat() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 select-none">
      {/* Primary Floating Button (WhatsApp Styled Green/Emerald) */}
      <motion.button
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.95 }}
        onClick={handleRedirect}
        className="w-14 h-14 bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-xl hover:bg-emerald-600 transition-colors z-50 relative group cursor-pointer"
        id="whatsapp-floater"
        title="Chat on WhatsApp"
        aria-label="Chat on WhatsApp"
      >
        <span className="absolute inset-0 rounded-full bg-emerald-500 opacity-20 animate-ping group-hover:hidden" />
        <svg
          viewBox="0 0 24 24"
          className="w-7.5 h-7.5 fill-white relative z-10 flex-shrink-0"
